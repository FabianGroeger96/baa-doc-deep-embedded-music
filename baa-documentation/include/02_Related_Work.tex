...
\end{equation}
The process of calculating the output of a \gls{NN} is called forward propagation. In deeper networks, the output of one layer is then used again as the input of the next layer $f^L(f^{L-1}(\cdots f^1(z)))$, where $L$ denotes the number of layers in the \gls{NN}. A layer can be thought of as a single perceptron, thus this is often called a \gls{MLP} which is illustrated in figure \ref{fig:MLP-Visualisation}.
