...
\begin{equation}
    \centering
    \begin{gathered}
        \Big|\Big|f(x_i^a) - f(x_i^p)\Big|\Big|_2^2 + \alpha < \Big|\Big|f(x_i^a) - f(x_i^n)\Big|\Big|_2^2 \\[10pt]
        \forall (f(x_i^a), f(x_i^p) f(x_i^n)) \in \tau
    \end{gathered}
    \label{eq:Triplet-Loss-Constraint}
\end{equation}
The embedding of a given sample is represented by $f(x) \in \mathbb{R}^d$. It embeds a sample $x$ into a $d$-dimensional Euclidean space. The loss, which will be minimized, is defined by the equation \ref{eq:Triplet-Loss}. Where $\alpha$ is a margin that is enforced between positive and negative pairs. $\tau$ is the set of all possible triplets in the training set and has cardinality $N$.
\myequations{Triplet Loss function}
\begin{equation}
    \centering
    L = \sum^N_{i = 1} \Bigg [\Big|\Big|f(x_i^a) - f(x_i^p)\Big|\Big|_2^2 - \Big|\Big|f(x_i^a) - f(x_i^n)\Big|\Big|_2^2 + \alpha \Bigg]_+
    \label{eq:Triplet-Loss}
\end{equation}
When the loss is minimized, it pushes the distance between $f(x_i^a)$ and $f(x_i^p)$ near zero. Contrarily it forces the distance between $f(x_i^a)$ and $f(x_i^n)$ to be greater than the distance between $f(x_i^a)$ and $f(x_i^p)$ plus a margin $\alpha$.
\newline
\newline
Generating all possible triplets would result in many triplet that are easily satisfied, which satisfy the equation \ref{eq:Triplet-Loss-Constraint}. Which would not contribute to the training and result in slower convergence, as they would still be passed through the network. It is crucial to select hard triplets, which can contribute to improving the model.
\newline
\newline
...
\begin{equation}
    \centering
    \text{hard positive} = \text{argmax}_{x_i^p} \ \Big|\Big|f(x_i^a) - f(x_i^p)\Big|\Big|_2^2
    \label{eq:Triplet-Loss-Hard-Positive}
\end{equation}
\myequations{Triplet Loss hard negative}
\begin{equation}
    \centering
    \text{hard negative} = \text{argmin}_{x_i^n} \ \Big|\Big|f(x_i^a) - f(x_i^n)\Big|\Big|_2^2
