...
\newline
\newline
The figure \ref{fig:Triplet-Loss-Music} shows the line plot of the triplet loss value of the trained epochs. It shows that the model starts with a relatively high loss value and decreases over time. At the end of the training, at epoch 130, the model reached a triplet loss value of approximately 0.35. The line plot further shows that the model could potentially profit from additional training with a lower learning rate to get an even lower triplet loss value.
\begin{figure}[ht]
\centering
...
The music embedding space is carefully examined, just like the noise detection space (\ref{sub:Eval-Embedding-Space-DCASE}). For the examination, the entire dataset is projected onto the embedding space and the value, including the metadata of each point in the embedding space, is saved. The first part of the examination is to examine the neighbourhood of the embedding space, which is done by building a neighbouring tree using the \texttt{cKDTree} from \texttt{scipy}. Then to examine the neighbourhood, from each of the embedded points its nearest 30 neighbours are checked, and their labels are compared, which aims to find inconsistencies. If a sample has 28 or more different labels in its neighbourhood, it is further examined, since this represents either a malfunction of the embedding space or a significant sample in the dataset, which can be classified as a different genre.
